...
namespace CodeWalker
{
    public partial class WorldForm : Form, DXForm
    {
        ...
        public void UpdateNavYnvGraphics(YnvFile ynv, bool fullupdate)
        {
            ...
            // Synchronous rebuild — queued Invalidate sometimes leaves a stale GPU buffer
            // for one or more frames when polys are deleted, producing a visible ghost.
            Renderer.InvalidateImmediate(ynv);
        }
        ...
        private void UpdateMouseHits(List<YnvFile> ynvs)
        {
            ...
            foreach (var ynv in ynvs)
            {
                ...
                if (ynv.Polys != null)
                {
                    UpdateMouseHitsForNavPolys(ynv, ref mray);
                }
            }

        }
        private void UpdateMouseHitsForNavPolys(YnvFile ynv, ref Ray mray)
        {
            // Iterate live polys + their local Vertices[] / CellAABB. The previous
            // implementation walked SectorTree.PolyIDs and dereferenced ynv.Indices/
            // ynv.Vertices, which only refresh during Save()/BuildStructs, so any
            // in-memory edit (move, delete, add) produced stale hit results.
            float hitdist;
            for (int i = 0; i < ynv.Polys.Count; i++)
            {
                var poly = ynv.Polys[i];
                if (poly?.Vertices == null || poly.Vertices.Length < 3) continue;

                var aabb = poly._RawData.CellAABB;
                var bbox = new BoundingBox(aabb.Min, aabb.Max);
                float bhd;
                if (!mray.Intersects(ref bbox, out bhd)) continue;
                ...
                for (int t = 0; t < tricount; t++)
                {
                    var p1 = verts[t + 1];
                    var p2 = verts[t + 2];
                    if (mray.Intersects(ref p0, ref p1, ref p2, out hitdist)
                        && (hitdist < CurMouseHit.HitDist) && (hitdist > 0))
                    {
