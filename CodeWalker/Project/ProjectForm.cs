...
namespace CodeWalker.Project
{
    public partial class ProjectForm : Form
    {
        ...
        public void AddYnvToProject(YnvFile ynv)
        {
            ...
            // If this is the YnvFile held by GameFileCache (i.e. came from the world view),
            // clone it so editing the project copy doesn't mutate the cached RPF version —
            // otherwise closing the project would leave the player's edits showing on the
            // original world ynv until the cache evicts it.
            if (IsGameFileCacheYnvInstance(ynv))
            {
                var clone = CloneYnvForProject(ynv);
                if (clone != null)
                {
                    // Drop both selection paths so the widget doesn't stay anchored to
                    // a poly inside the (now read-only) cached YnvFile. If we don't do
                    // this the user's first drag silently moves a polygon in the cached
                    // copy — and only the second click, which re-hit-tests and picks the
                    // cloned poly, actually edits the project.
                    CurrentNavPoly = null;
                    WorldForm?.SelectItem(null);
                    ynv = clone;
                }
        ...
        private bool IsGameFileCacheYnvInstance(YnvFile ynv)
        {
            if (ynv == null || GameFileCache == null || !GameFileCache.IsInited) return false;
            var name = ynv.Name;
            if (string.IsNullOrEmpty(name)) return false;
            // YnvDict is keyed by ShortNameHash — hash the name without the .ynv extension.
            uint hash = JenkHash.GenHash(Path.GetFileNameWithoutExtension(name));
            var cached = GameFileCache.GetYnv(hash);
            return ReferenceEquals(cached, ynv);
        }

        private YnvFile CloneYnvForProject(YnvFile src)
        {
            ...
            byte[] data;
            bool prevBuild = src.BuildStructsOnSave;
            try
            {
                src.BuildStructsOnSave = false;
                data = src.Save();
            }
            finally
            {
                src.BuildStructsOnSave = prevBuild;
            }
            if (data == null) return null;

            var clone = new YnvFile();
            clone.Load(data);
            clone.RpfFileEntry.Name = name;
            clone.Name = name;

            string projDir = (CurrentProjectFile != null && !string.IsNullOrEmpty(CurrentProjectFile.Filepath))
                ? new FileInfo(CurrentProjectFile.Filepath).DirectoryName
                : null;
            if (!string.IsNullOrEmpty(projDir))
            {
                string filepath = Path.Combine(projDir, name);
                try
                {
                    File.WriteAllBytes(filepath, data);
                ...
                {
                    // saving failed — keep clone in-memory only
                }
            }
            ...
            if (WorldForm != null)
            {
                WorldForm.UpdateNavYnvGraphics(clone, true);
            }
            return clone;
        }
        // Like AddYnvToProject, but also auto-imports the 8 surrounding ynv files
        // from RPF so the user can edit any of them in the project, and so the later
        // Recalculate Edge Links pass has every neighbour already loaded.
        public void AddYnvToProjectWithNeighbours(YnvFile ynv)
        {
            if (ynv == null) return;
            if (CurrentProjectFile == null)
            {
            ...
            {
                MessageBox.Show(
                    "Save the project first (File → Save Project).\n\n" +
                    "Neighbour .ynv files imported from RPF need a project folder " +
                    "on disk so they can be written next to your other project files.",
                    "Add to Project");
                return;
            }

            // Centre: clone if it's the cached RPF instance, then add.
            if (!YnvExistsInProject(ynv))
            {
                if (IsGameFileCacheYnvInstance(ynv))
                {
                    var clone = CloneYnvForProject(ynv);
            ...
            // 8 surrounding cells in the V navmesh grid (file step = 3).
            int cx = ynv.CellX;
            int cy = ynv.CellY;
            ...
            var importedFromRpf = new List<string>();
            ...
            foreach (var off in offsets)
            {
                ...
                GetOrImportNeighbourYnv(nx, ny, importedFromRpf, missing);
            }
            ...
            if (missing.Count > 0)
            {
                ...
                sb.AppendLine("Added " + ynv.Name + " and " + importedFromRpf.Count + " neighbour(s) to the project.");
                sb.AppendLine();
                sb.AppendLine("Missing in RPF (skipped):");
                foreach (var n in missing) sb.AppendLine("  " + n);
                MessageBox.Show(sb.ToString(), "Add to Project");
            }
        }
        ...
        public void RecalculateYnvEdgeLinks()
        {
            if ((CurrentYnvFile == null) && (CurrentNavPoly != null)) CurrentYnvFile = CurrentNavPoly.Ynv;
            if (CurrentYnvFile == null) return;
            if (CurrentProjectFile == null) return;
            if (!YnvExistsInProject(CurrentYnvFile))
            {
                MessageBox.Show("Add the .ynv to the project first.");
                return;
            }
            if (string.IsNullOrEmpty(CurrentProjectFile.Filepath))
            {
                MessageBox.Show(
                    ...
                    "Neighbour .ynv files imported from RPF need a project folder " +
                    "on disk so they can be written next to your other project files.",
                    "Recalculate Edge Links");
                return;
            }

            var ynv = CurrentYnvFile;
            int cx = ynv.CellX;
            int cy = ynv.CellY;

            // 8-cell neighbourhood. Diagonal cells can share corner vertices, but the
            // sector grid in V is rectilinear so diagonals contribute only if rare
            // corner-touching polys exist; including them is harmless.
            var offsets = new (int dx, int dy)[]
            {
                (-1,-1), ( 0,-1), ( 1,-1),
                (-1, 0),          ( 1, 0),
            ...
            var neighbours = new List<YnvFile>();
            var importedFromRpf = new List<string>();
            var missing = new List<string>();
            foreach (var off in offsets)
            {
                ...
                var nynv = GetOrImportNeighbourYnv(nx, ny, importedFromRpf, missing);
                if (nynv != null) neighbours.Add(nynv);
            }

            RecalcEdgeLinksResult result;
            lock (projectsyncroot)
            {
                result = ynv.RecalculateEdgeLinks(neighbours);
            }

            SetYnvHasChanged(true);
            // Refresh the project tree if anything was added or its HasChanged state changed.
            if ((result.ChangedNeighbours.Count > 0) || (importedFromRpf.Count > 0))
            {
                LoadProjectTree();
            }

            var sb = new StringBuilder();
            sb.AppendLine("Recalculated edge links for " + ynv.Name + ":");
            sb.AppendLine("  Internal edge pairs: " + result.InternalLinks);
            sb.AppendLine("  Cross-sector edge pairs: " + result.CrossSectorLinks);
            sb.AppendLine("  Unresolved boundary edges: " + result.BoundaryEdges);
            sb.AppendLine("  Neighbour ynvs updated: " + result.ChangedNeighbours.Count);
            if (importedFromRpf.Count > 0)
            {
                sb.AppendLine();
                sb.AppendLine("Imported from RPF into project:");
                foreach (var n in importedFromRpf) sb.AppendLine("  " + n);
            }
            if (missing.Count > 0)
            {
                sb.AppendLine();
            ...
            MessageBox.Show(sb.ToString(), "Recalculate Edge Links");
        }

        private YnvFile GetOrImportNeighbourYnv(int cx, int cy, List<string> importedFromRpf, List<string> missing)
        {
            // V navmesh files are placed on a 3-step grid: navmesh[X*3][Y*3].ynv where
            // X/Y are 0..99 (the cx/cy passed in here are already in that grid space).
            // See SpaceNavGridCell.FileX = x*3 in Space.cs.
            string name = "navmesh[" + (cx * 3) + "][" + (cy * 3) + "].ynv";

            // Already in the project?
            foreach (var pynv in CurrentProjectFile.YnvFiles)
            {
                if (pynv == null) continue;
                if (string.Equals(pynv.Name, name, StringComparison.OrdinalIgnoreCase))
                {
                    if (!pynv.Loaded && !string.IsNullOrEmpty(pynv.FilePath) && File.Exists(pynv.FilePath))
                    {
                        try { LoadYnvFromFile(pynv, pynv.FilePath); } catch { }
                    }
                    return pynv.Loaded ? pynv : null;
                }
            }
            ...
            // GameFileCache.YnvDict is keyed by ShortNameHash (no extension).
            uint hash = JenkHash.GenHash(Path.GetFileNameWithoutExtension(name));
            var src = GameFileCache.GetYnv(hash);
            if (src == null)
            {
                missing.Add(name);
                return null;
            ...
            // GetYnv may have just queued the load — block briefly until it finishes.
            var sw = System.Diagnostics.Stopwatch.StartNew();
            while (!src.Loaded && (sw.ElapsedMilliseconds < 5000))
            {
                Thread.Sleep(15);
            }
            if (!src.Loaded)
            {
                missing.Add(name + " (load timeout)");
                return null;
            }

            // Round-trip the source through Save()/Load() so we don't mutate the cache copy.
            byte[] data;
            bool prevBuild = src.BuildStructsOnSave;
            try
            {
            ...
            if (string.IsNullOrEmpty(projDir))
            {
                missing.Add(name + " (project not saved)");
                return null;
            }
            ...
            nynv.FilePath = filepath;
            JenkIndex.Ensure(Path.GetFileNameWithoutExtension(name));

            if (!CurrentProjectFile.AddYnvFile(nynv))
            {
                // Already added under a different code path — keep going.
                return nynv;
            }

            // Flag as changed so SaveAll / CloseProject prompt the user to save this
            // freshly-imported neighbour, even when RecalculateEdgeLinks doesn't end up
            // patching any of its cross-sector edges.
            nynv.HasChanged = true;
            importedFromRpf.Add(name);
            CurrentProjectFile.HasChanged = true;
            if (WorldForm != null)
            {
                WorldForm.UpdateNavYnvGraphics(nynv, true);
            }
            return nynv;
        }
        ...
        {
            // Real removal from the Polys list (Blender's plain Delete workflow):
            // RemovePoly renumbers every internal Index, patches Edge AreaID/PolyID
            // references inside this ynv, drops portals tied to the removed poly,
            // and re-keys every YnvPoly.PortalLinks[] against the new portal list.
            // For cross-sector references in neighbouring ynvs the user is expected
            // to run Ynv → Recalculate Edge Links after a batch of edits — that pass
            // sees the new indices and rewires sibling edges to match.
            if (CurrentYnvFile == null) return false;
            if (CurrentNavPoly == null) return false;
            if (CurrentNavPoly.Ynv != CurrentYnvFile) return false;
            if (CurrentYnvFile.Polys == null) return false;
            ...
            if (WorldForm != null)
            {
                lock (WorldForm.RenderSyncRoot)
                {
                    res = CurrentYnvFile.RemovePoly(delpoly);
                }
            }
            else
            {
            ...
            if (WorldForm != null)
            {
                WorldForm.UpdateNavYnvGraphics(CurrentYnvFile, true);
                WorldForm.SelectItem(null);
            }
            return true;
        }
        ...
        private void RefreshYnvUI()
        {
            ...
            YnvRecalcLinksMenu.Enabled = enable && inproj;
        ...
        private void YnvRecalcLinksMenu_Click(object sender, EventArgs e)
        {
            RecalculateYnvEdgeLinks();
        }
        private void YnvAddToProjectMenu_Click(object sender, EventArgs e)
        {
            AddYnvToProjectWithNeighbours(CurrentYnvFile);
        }
