...
namespace CodeWalker.Rendering
{
    public class RenderableCache
    {
        ...
        // Synchronously rebuild the GPU buffer for a path-data item right now.
        // Use after editing nav mesh poly geometry / removing polys, where the
        // queued Invalidate path is sometimes too late for the next frame.
        public void InvalidateImmediate(BasePathData path)
        {
            if (currentDevice == null) return;
            pathbatches.UpdateImmediate(path, currentDevice);
        }
