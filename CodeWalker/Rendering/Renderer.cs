...
namespace CodeWalker.Rendering
{
    public class Renderer
    {
        ...
        public void InvalidateImmediate(BasePathData path)
        {
            renderableCache.InvalidateImmediate(path);
        }
        ...
        {
            // Use the polygon's local Vertices[] (which is what gets translated by
            // YnvPoly.SetPosition / vertex edits), not the YnvFile-global Vertices/Indices
            // — those don't update until Save() rebuilds the structs, so reading them
            // while editing produces a "ghost" selection at the original position.
            var verts = poly.Vertices;
            if ((verts == null) || (verts.Length < 3)) return;

            var pcolour = new Color4(1.0f, 1.0f, 1.0f, 0.2f);
            var colourval = (uint)pcolour.ToRgba();
            VertexTypePC v0 = new VertexTypePC { Colour = colourval, Position = verts[0] };
            VertexTypePC v1 = new VertexTypePC { Colour = colourval };
            VertexTypePC v2 = new VertexTypePC { Colour = colourval };
        ...
        {
            // See RenderSelectionNavPoly for why we don't go through ynv.Indices/Vertices.
            var verts = poly.Vertices;
            if ((verts == null) || (verts.Length < 2)) return;

            VertexTypePC v = new VertexTypePC { Colour = colourval };
            VertexTypePC v0 = new VertexTypePC { Colour = colourval, Position = verts[0] };
