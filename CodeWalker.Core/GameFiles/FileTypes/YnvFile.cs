...
namespace CodeWalker.GameFiles
{
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvFile : GameFile, PackedFile, BasePathData
    {
        ...
        // Quantization for vertex matching when rebuilding edge adjacency.
        // 2 decimals == 1 cm. Sollumz works at 1 mm because Blender loads both
        // neighbours from the same XML at float32 precision, so shared boundary
        // vertices match bit-exactly. In CodeWalker we operate on the raw binary
        // where each sector stores its own int16-packed vertex relative to its
        // own Nav.AABBSize (~150 m ÷ 65535 ≈ 2.3 mm/unit), and a shared world-space
        // vertex can drift by a couple of millimetres between two neighbouring
        // sectors. 1 cm tolerance easily absorbs that and is still far below the
        // distance between any two real navmesh edges (poly edges are metres apart).
        internal const double VertexQuantizeScale = 100.0;

        internal struct VertKey : IEquatable<VertKey>
        {
            public int X, Y, Z;
            public static VertKey From(Vector3 v) => new VertKey
            {
                X = (int)Math.Round(v.X * VertexQuantizeScale),
                Y = (int)Math.Round(v.Y * VertexQuantizeScale),
                Z = (int)Math.Round(v.Z * VertexQuantizeScale),
            };
            public bool Equals(VertKey o) => X == o.X && Y == o.Y && Z == o.Z;
            public override bool Equals(object o) => o is VertKey k && Equals(k);
        ...
        {
            public VertKey A, B;
            public static EdgeKey From(Vector3 va, Vector3 vb)
            {
                var ka = VertKey.From(va);
                var kb = VertKey.From(vb);
                if (CompareKeys(ka, kb) <= 0) return new EdgeKey { A = ka, B = kb };
                return new EdgeKey { A = kb, B = ka };
            }
            private static int CompareKeys(VertKey a, VertKey b)
            {
            ...
            public bool Equals(EdgeKey o) => A.Equals(o.A) && B.Equals(o.B);
        ...
        /// <summary>
        /// Rebuilds <see cref="YnvEdge.Poly1"/>/<see cref="YnvEdge.Poly2"/> and the
        /// AreaID/PolyID values stored on every polygon corner. Matches edges by
        /// quantized vertex positions so neighbouring polygons that share a boundary
        /// (in this YnvFile or in any of the supplied <paramref name="neighbours"/>)
        /// get cross-referenced. Boundary edges with no match collapse to 16383:16383.
        /// </summary>
        public RecalcEdgeLinksResult RecalculateEdgeLinks(IEnumerable<YnvFile> neighbours = null)
        {
            var result = new RecalcEdgeLinksResult();
            if (Polys == null) return result;

            uint selfArea = (uint)AreaID;
            ushort selfAreaU16 = (ushort)AreaID;
            for (int i = 0; i < Polys.Count; i++)
            {
                var p = Polys[i];
                p.Index = i;
                p.AreaID = selfAreaU16;
                p.Ynv = this;
            }

            var selfMap = BuildEdgeMap(this);

            // Walk every corner of every polygon and resolve the (Poly1, Poly2) pair.
            // Pre-build neighbour edge maps & area→ynv lookup so we can keep stored
            // adjacency for sectors that aren't loaded (Blender's "trust verbatim" rule).
            var neighbourMaps = new List<(YnvFile Ynv, Dictionary<EdgeKey, PolyCorner> Map)>();
            var loadedAreaIds = new HashSet<uint>();
            if (neighbours != null)
            {
                foreach (var n in neighbours)
                {
                    if (n == null || n == this || n.Polys == null) continue;
                    ushort nArea = (ushort)n.AreaID;
                    for (int i = 0; i < n.Polys.Count; i++)
                    {
                        var p = n.Polys[i];
                        p.Index = i;
                        p.AreaID = nArea;
                    ...
                    neighbourMaps.Add((n, BuildEdgeMapFirstOnly(n)));
                    loadedAreaIds.Add((uint)n.AreaID);
                }
            }
            ...
            for (int i = 0; i < Polys.Count; i++)
            {
                ...
                int vc = poly.Vertices.Length;
                if (vc < 2) continue;

                EnsureEdgesArray(poly, vc);

                for (int c = 0; c < vc; c++)
                {
                    var e = poly.Edges[c];

                    // Snapshot the stored neighbour reference. Compare against AreaID2
                    // because that's the half that Init() pulled out of binary; we'll
                    // use it as fallback when no live match is found below.
                    uint storedArea = (e.AreaID2 != 0) ? e.AreaID2 : e.AreaID1;
                    uint storedPoly = (e.PolyID2 != 0) ? e.PolyID2 : e.PolyID1;

                    e.Ynv = this;

                    var key = EdgeKey.From(poly.Vertices[c], poly.Vertices[(c + 1) % vc]);

                    // 1. Internal neighbour inside this YnvFile.
                    YnvPoly other = FindOtherInSameYnv(selfMap, key, poly, c);
                    if (other != null)
                    {
                        SetEdgeNeighbour(e, other);
                        result.InternalLinks++;
                        continue;
                    }

                    // 2. Cross-sector neighbour in a *loaded* sibling YnvFile.
                    bool foundCross = false;
                    foreach (var pair in neighbourMaps)
                    {
                        if (!pair.Map.TryGetValue(key, out var match)) continue;

                        SetEdgeNeighbour(e, match.Poly);

                        // Reciprocal link on the neighbour's corner — its edge points
                        // back at OUR poly. Both halves of the pair hold our (area, poly).
                        EnsureEdgesArray(match.Poly, match.Poly.Vertices.Length);
                        var ne = match.Poly.Edges[match.Corner];
                        ne.Ynv = pair.Ynv;
                        SetEdgeNeighbour(ne, poly);

                        pair.Ynv.HasChanged = true;
                        if (!result.ChangedNeighbours.Contains(pair.Ynv))
                            result.ChangedNeighbours.Add(pair.Ynv);
                        result.CrossSectorLinks++;
                        foundCross = true;
                        break;
                    }
                    if (foundCross) continue;

                    // 3. No live match found.
                    //  - stored area was this ynv (the sibling was internal and got
                    //    deleted) → write NONE; the old PolyID is invalid and would
                    //    crash the game's pathfinder.
                    //  - stored area belongs to a sibling we DID load yet still no
                    //    edge matched → its polygon was deleted/moved; write NONE.
                    //  - stored area belongs to a sibling we did NOT load → trust
                    //    the value verbatim (Blender's rule). This preserves real
                    //    cross-sector links to areas outside the recalc set.
                    //  - stored already NONE → leave NONE.
                    e.Poly1 = null;
                    e.Poly2 = null;
                    if (storedArea == 0x3FFF || storedArea == selfArea || loadedAreaIds.Contains(storedArea))
                    {
                        e.AreaID1 = e.AreaID2 = 0x3FFF;
                        e.PolyID1 = e.PolyID2 = 0x3FFF;
                    }
                    else
                    {
                        // Sibling cell not loaded — preserve the cross-sector link the
                        // file shipped with. Same pair value on both halves (game's
                        // expected format: each corner's edge entry duplicates the
                        // single (neighbour_area, neighbour_poly) tuple).
                        e.AreaID1 = e.AreaID2 = storedArea;
                        e.PolyID1 = e.PolyID2 = storedPoly;
                    }
                }
            }

            // Count remaining boundary edges that stayed unresolved.
            foreach (var poly in Polys)
            {
                if (poly.Edges == null) continue;
                foreach (var e in poly.Edges)
                {
                    if (e != null && e.AreaID2 == 0x3FFF) result.BoundaryEdges++;
                }
            }

            // Validate portals against the *current* poly count in every affected ynv.
            // Portals pointing at a deleted polygon would crash the game at path planning.
            PrunePortals(this);
            foreach (var n in result.ChangedNeighbours)
            {
                PrunePortals(n);
            }
        ...
        private static void PrunePortals(YnvFile ynv)
        {
            if (ynv?.Portals == null) return;
            int polyCount = ynv.Polys?.Count ?? 0;
            ushort selfArea = (ushort)ynv.AreaID;

            // Collect old-index → new-index mapping while we remove invalid portals.
            var indexMap = new Dictionary<int, int>(ynv.Portals.Count);
            var kept = new List<YnvPortal>(ynv.Portals.Count);
            for (int i = 0; i < ynv.Portals.Count; i++)
            {
                var p = ynv.Portals[i];
                if (p == null) continue;
                bool badFrom = (p._RawData.AreaIDFrom == selfArea)
                               && (p._RawData.PolyIDFrom1 >= polyCount)
                               && (p._RawData.PolyIDFrom1 != 0x3FFF);
                bool badTo = (p._RawData.AreaIDTo == selfArea)
                             && (p._RawData.PolyIDTo1 >= polyCount)
                             && (p._RawData.PolyIDTo1 != 0x3FFF);
                if (badFrom || badTo) continue;
                indexMap[i] = kept.Count;
                kept.Add(p);
            }

            if (kept.Count == ynv.Portals.Count) return; // nothing to do

            ynv.Portals = kept;
            for (int i = 0; i < kept.Count; i++) kept[i].Index = i;
            RemapPortalLinks(ynv, indexMap);
            ynv.HasChanged = true;
        }

        /// <summary>
        /// Update every YnvPoly.PortalLinks[] in <paramref name="ynv"/> after the
        /// portal list has been compacted. Entries whose old index is missing from
        /// the map (portal got removed) are dropped; remaining entries get
        /// renumbered. Without this fix the game reads stale indices on load and
        /// crashes its pathfinder.
        /// </summary>
        private static void RemapPortalLinks(YnvFile ynv, Dictionary<int, int> oldToNew)
        {
            if (ynv?.Polys == null) return;
            foreach (var poly in ynv.Polys)
            {
                if (poly?.PortalLinks == null || poly.PortalLinks.Length == 0) continue;
                var list = new List<ushort>(poly.PortalLinks.Length);
                foreach (var oldIdx in poly.PortalLinks)
                {
                    if (oldToNew.TryGetValue(oldIdx, out int newIdx))
                    {
                        list.Add((ushort)newIdx);
                    }
                }
                poly.PortalLinks = list.Count > 0 ? list.ToArray() : null;
            }
        }

        private static Dictionary<EdgeKey, List<PolyCorner>> BuildEdgeMap(YnvFile ynv)
        {
            var map = new Dictionary<EdgeKey, List<PolyCorner>>();
            foreach (var poly in ynv.Polys)
            {
                if (poly.Vertices == null) continue;
                int vc = poly.Vertices.Length;
                for (int c = 0; c < vc; c++)
                {
                    ...
                    {
                        list = new List<PolyCorner>(2);
                        map[key] = list;
                    }
                    list.Add(new PolyCorner { Poly = poly, Corner = c });
                }
            }
            return map;
        }

        private static Dictionary<EdgeKey, PolyCorner> BuildEdgeMapFirstOnly(YnvFile ynv)
        {
            var map = new Dictionary<EdgeKey, PolyCorner>();
            foreach (var poly in ynv.Polys)
            {
                if (poly.Vertices == null) continue;
                int vc = poly.Vertices.Length;
                ...
                {
                    var key = EdgeKey.From(poly.Vertices[c], poly.Vertices[(c + 1) % vc]);
                    if (!map.ContainsKey(key)) map[key] = new PolyCorner { Poly = poly, Corner = c };
                }
            }
            return map;
        }

        private static YnvPoly FindOtherInSameYnv(Dictionary<EdgeKey, List<PolyCorner>> map, EdgeKey key, YnvPoly self, int selfCorner)
        {
            if (!map.TryGetValue(key, out var list)) return null;
            foreach (var pc in list)
            {
                if (pc.Poly != self || pc.Corner != selfCorner) return pc.Poly;
            }
            return null;
        }

        // Game .ynv format stores each polygon-corner edge as a duplicated
        // (neighbour_area, neighbour_poly) tuple — i.e. AreaID1 == AreaID2 and
        // PolyID1 == PolyID2, both referencing the neighbour across this edge.
        // Writing (self, neighbour) instead causes the game's pathfinder to read
        // a non-existent two-sided edge and crash.
        private static void SetEdgeNeighbour(YnvEdge e, YnvPoly neighbour)
        {
            e.Poly1 = neighbour;
            e.Poly2 = neighbour;
            e.AreaID1 = neighbour.AreaID;
            e.AreaID2 = neighbour.AreaID;
            e.PolyID1 = (uint)neighbour.Index;
            e.PolyID2 = (uint)neighbour.Index;
        }

        private static void EnsureEdgesArray(YnvPoly poly, int vc)
        {
            if (poly.Edges == null || poly.Edges.Length != vc)
            {
                var arr = new YnvEdge[vc];
                for (int i = 0; i < vc; i++)
                {
                    arr[i] = (poly.Edges != null && i < poly.Edges.Length && poly.Edges[i] != null)
                        ? poly.Edges[i]
                        : new YnvEdge();
                }
                poly.Edges = arr;
            ...
            else
            {
                for (int i = 0; i < vc; i++)
                {
                    if (poly.Edges[i] == null) poly.Edges[i] = new YnvEdge();
                }
            }
        }
        ...
        /// <summary>
        /// Removes a polygon from this YnvFile, renumbers the remaining polygons' Index,
        /// and patches any edge in this YnvFile that referenced the removed poly to point
        /// to "no neighbour" (16383:16383). Cross-YnvFile references are intentionally not
        /// touched here — run <see cref="RecalculateEdgeLinks"/> afterwards if you need
        /// neighbour ynvs to be patched as well.
        /// </summary>
        public bool RemovePoly(YnvPoly poly)
        {
            if (poly == null || Polys == null) return false;
            int removedIndex = Polys.IndexOf(poly);
            if (removedIndex < 0) return false;

            Polys.RemoveAt(removedIndex);

            ushort selfArea = (ushort)AreaID;
            for (int i = 0; i < Polys.Count; i++)
            {
                var p = Polys[i];
                p.Index = i;
                ...
                foreach (var e in p.Edges)
                {
                    ...
                    else if (e.AreaID1 == selfArea && e.PolyID1 > removedIndex && e.PolyID1 != 0x3FFF)
                    {
                        e.PolyID1 = e.PolyID1 - 1;
                    }
                    if (e.Poly2 == poly)
                    {
                        e.Poly2 = null;
                    ...
                    else if (e.AreaID2 == selfArea && e.PolyID2 > removedIndex && e.PolyID2 != 0x3FFF)
                    {
                        e.PolyID2 = e.PolyID2 - 1;
                    }
                }
            }

            // Portals referencing the removed poly must go too — otherwise the
            // exported file has a portal whose PolyIDFrom/To is out of range and
            // the game crashes when its pathfinder touches that sector. We also
            // need every surviving polygon's PortalLinks[] re-keyed against the
            // new portal indices (stale indices = the same crash on game load).
            if (Portals != null)
            {
                var indexMap = new Dictionary<int, int>(Portals.Count);
                var kept = new List<YnvPortal>(Portals.Count);
                for (int pi = 0; pi < Portals.Count; pi++)
                {
                    var pt = Portals[pi];
                    if (pt == null) continue;
                    var raw = pt._RawData;
                    bool fromHere = raw.AreaIDFrom == selfArea;
                    bool toHere = raw.AreaIDTo == selfArea;
                    bool drop =
                        (fromHere && raw.PolyIDFrom1 == removedIndex) ||
                        (toHere && raw.PolyIDTo1 == removedIndex);
                    if (drop) continue;

                    bool dirty = false;
                    if (fromHere && raw.PolyIDFrom1 > removedIndex && raw.PolyIDFrom1 != 0x3FFF)
                    {
                        raw.PolyIDFrom1--;
                        raw.PolyIDFrom2 = raw.PolyIDFrom1;
                        dirty = true;
                    }
                    if (toHere && raw.PolyIDTo1 > removedIndex && raw.PolyIDTo1 != 0x3FFF)
                    {
                        raw.PolyIDTo1--;
                        raw.PolyIDTo2 = raw.PolyIDTo1;
                        dirty = true;
                    }
                    if (dirty) pt._RawData = raw;
                    indexMap[pi] = kept.Count;
                    kept.Add(pt);
                }
                if (kept.Count != Portals.Count)
                {
                    Portals = kept;
                    for (int pi = 0; pi < kept.Count; pi++) kept[pi].Index = pi;
                    RemapPortalLinks(this, indexMap);
                }
                else
                {
                    for (int pi = 0; pi < kept.Count; pi++) kept[pi].Index = pi;
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvPoly : IMetaXmlItem
    {
        ...
        public void SetPosition(Vector3 pos)
        {
            ...
            Position = pos;
            if (Vertices == null) { CalculateAABB(); return; }

            // Snapshot the vertices we're about to move so we can find every neighbour
            // poly that shares any of them and drag those shared verts along — this
            // mirrors Blender's welded-mesh behaviour (one shared vertex == one
            // vertex). Without it, moving a polygon tears its boundary away from
            // every neighbour: in the saved file two vertices that used to occupy
            // the same point now sit at different positions, the polys end up
            // disconnected, and the in-game pathfinder treats this poly as an
            // island — peds stand still on it because no edge crosses the gap.
            var oldVerts = new Vector3[Vertices.Length];
            for (int i = 0; i < Vertices.Length; i++) oldVerts[i] = Vertices[i];

            for (int i = 0; i < Vertices.Length; i++) Vertices[i] += delta;
            CalculateAABB();

            if (Ynv?.Polys == null) return;
            // Hash each shared vertex by quantized position so the match is robust
            // to the int16→float32 round-trip and to small movements we've already
            // applied. Use the same tolerance as edge matching.
            var moved = new HashSet<YnvFile.VertKey>();
            for (int i = 0; i < oldVerts.Length; i++) moved.Add(YnvFile.VertKey.From(oldVerts[i]));

            foreach (var p in Ynv.Polys)
            {
                if (p == this || p.Vertices == null) continue;
                bool anyMoved = false;
                for (int i = 0; i < p.Vertices.Length; i++)
                {
                    if (moved.Contains(YnvFile.VertKey.From(p.Vertices[i])))
                    {
                        p.Vertices[i] += delta;
                        anyMoved = true;
                    }
                }
                if (anyMoved) p.CalculateAABB();
            }
        }
    ...
    public class RecalcEdgeLinksResult
    {
        public int InternalLinks;     // edge pairs matched inside the current YnvFile
        public int CrossSectorLinks;  // edge pairs matched against a neighbour YnvFile
        public int BoundaryEdges;     // edges left as 16383:16383 (no match found anywhere)
        public List<YnvFile> ChangedNeighbours = new List<YnvFile>();
    }
